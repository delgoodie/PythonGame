...
class Map:
    def __init__(self, map_path, game):
        ...
        for y in range(len(self.map)):
            ...
            for x in range(len(self.map[y])):
                if self.map[y][x]:
                    self.collider_map[y].append(Collider("rect", Vec2(x, y), Vec2.one(), 1))
    ...
    def render(self, sprites: list[Sprite]):
        for y in range(len(self.map)):
            for x in range(len(self.map[y])):
                sprites.append(Sprite(self.wall_tile if self.map[y][x] else self.floor_tile, Vec2(x, y), 0, Vec2(1, 1), 10))
